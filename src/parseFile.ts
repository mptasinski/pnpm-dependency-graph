...
const createPackageName = (name: string, version: string, pkg = 'npm'): string => `pkg:${pkg}/${escapeName(name)}@${version}`

const parsePackageNameFromLockPackages = (nameString: string): string => {
    const [,namespace, packageName, version] = nameString.split('_')[0].split('/')
    return createPackageName(`${namespace.replace("@", "%40")}/${packageName}`, version)
}

const getOrCreatePackage = (map: Map<string, Package>, pkgName: string): Package => map.get(pkgName) || (map.set(pkgName, new Package(pkgName)) && map.get(pkgName) as Package)

const createDependenciesList = (packages: LockFile['packages']): Map<string, Package> => {
        ...
        .reduce((map, [name, data]) => {
            ...
            if(data.dependencies) {
                Object
                    ...
                    .forEach(([pkName, pkVersion]) => current.dependsOn(getOrCreatePackage(map, createPackageName(pkName, pkVersion))))
            }
...
export const parseFile = ({fileContent}: LoadedFile): Manifest[] => {
    ...
    return Object.entries(parsedFile.importers)
        .map(([workspace, definition]) => {
            const manifest = new Manifest(workspace === '.' ? 'root' : workspace, workspace)
            if (definition?.specifiers) {
                Object
                    .entries(definition.specifiers)
